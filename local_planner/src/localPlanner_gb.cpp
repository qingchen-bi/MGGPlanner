...
void laserCloudHandler(const sensor_msgs::PointCloud2ConstPtr& laserCloud2)
{
  if (!useTerrainAnalysis) {
    ...
    // tf::StampedTransform cloud_to_robot_transform;
    // try {
    //   listener_->lookupTransform(RobotID_TF_prefix+"base_link", laserCloud2->header.frame_id, ros::Time(0), cloud_to_robot_transform);
    // }
    ...
    for (int i = 0; i < laserCloudSize; i++) {
      ...
      float pointX = point.x; // cloudpoint_tf_robot[0];
      float pointY = point.y; // cloudpoint_tf_robot[1];
      float pointZ = point.z; //cloudpoint_tf_robot[2];
      ...
      if (dis < adjacentRange) {
        ...
        pointi.y = pointY;
        pointi.z = pointZ;
        laserCloudCrop->push_back(pointi);
      }
    }
...
int main(int argc, char** argv)
{
  ...
  nhPrivate.getParam("RobotID_TF_prefix", RobotID_TF_prefix);
    ...
    if (newLaserCloud || newTerrainCloud) {
      ...
      while (pathScale >= minPathScale && pathRange >= minPathRange) {
        ...
        if (selectedGroupID >= 0) {
          ...
          path.header.frame_id = RobotID_TF_prefix+"base_link";
      ...
      plannerCloud2.header.frame_id = RobotID_TF_prefix+"base_link";
