#include "mggplanner_ui.h"
// pci_initialization_trigger
namespace mggplanner_ui {

mggplanner_panel::mggplanner_panel(QWidget* parent) : rviz::Panel(parent) {
...
void mggplanner_panel::on_start_planner_click() {
  std_srvs::Trigger srv;
  if (!planner_client_start_planner.call(srv)) {
    ROS_ERROR("[MGGPLANNER-UI] Service call failed: %s",
              planner_client_start_planner.getService().c_str());
  }
}

void mggplanner_panel::on_stop_planner_click() {
  std_srvs::Trigger srv;
...
void mggplanner_panel::on_homing_click() {
  std_srvs::Trigger srv;
...
void mggplanner_panel::on_init_motion_click() {
...
void mggplanner_panel::on_plan_to_waypoint_click() {
  std_srvs::Trigger srv;
...
void mggplanner_panel::on_global_planner_click() {
  ...
  if (in_string.empty())
    ...
    try {
      ...
      ROS_ERROR("[MGGPLANNER UI] - Invalid ID: %s", in_string.c_str());
      return;
  ...
  if (id < 0) {
    ROS_ERROR("[MGGPLANNER UI] - In valid ID, must be non-negative");
    return;
  }
  // we got an ID!!!!!!!!!
...
void mggplanner_panel::save(rviz::Config config) const {
  rviz::Panel::save(config);
}
void mggplanner_panel::load(const rviz::Config& config) {
...
PLUGINLIB_EXPORT_CLASS(mggplanner_ui::mggplanner_panel, rviz::Panel)
