...
void PurePursuitController::actionGoalCallback(const planner_msgs::pathFollowerActionGoalConstPtr &goal)
{
	...
	while(ros::ok())
	{
		...
		{
			ROS_WARN("Bunker Path Tracker: Path tracking preempted, stopping");
			// Publish 0 velocity
		...
		{
			ROS_WARN("Bunker Path Tracker: Commanded path is empty, stopping");
			// Publish 0 velocity
			geometry_msgs::Twist vel;
			vel.linear.x = 0.0;
			vel.angular.z = 0.0;
		...
		{
			ROS_INFO("Bunker Path Tracker: Reached end of path");
		...
		{
			// std::cout << "cmd vel: " << cmd_vel_.linear.x << ", " << cmd_vel_.angular.z
			// 		  <<" Dist to goal"<< calculateDistance(path_to_exec_.back().pose, current_pose_) 
					//   << "Pose"<< current_pose_<< "goal"<< path_to_exec_.back().pose
					//   << std::endl;
			cmd_vel_pub_.publish(cmd_vel_);
		}
		else
		{
			ROS_WARN("Bunker Path Tracker: Can't track given path");
		}
...
bool PurePursuitController::computeCommandVelocity()
{
	...
	else
	{
		...
		// std::cout << "radius: " << radius << " min radius: " << radius_min<< " theta "<< delta_theta <<","<< critical_angle_<<","<<curvature_scaling_angle_ << std::endl;
		// double max_feasible_speed = std::min(current_vel_.linear.x + max_lin_acc_ * dt_, max_lin_vel_);
		// double min_feasible_speed = std::max(current_vel_.linear.x - max_lin_dec_ * dt_, -max_lin_vel_);
